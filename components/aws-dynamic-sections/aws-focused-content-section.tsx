...
type AwsFocusedContentSectionData = {
  heading?: string;
...
interface Props {
  data: AwsFocusedContentSectionData;
}

export default function AwsFocusedContentSection({ data }: Props) {
            ...
            >
              ...
              <ul className="space-y-3 px-5 py-4 font-medium">
                {card.items?.map((item, j) => (
                  <li
                    key={j}
                    className="flex items-center gap-2 text-secondary-700 text-sm md:text-base"
                  >
