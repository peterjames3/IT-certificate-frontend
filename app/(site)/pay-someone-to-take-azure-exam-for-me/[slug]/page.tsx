...
// ── Section components ──────────────────────────────────────
import HeroSection from "@/components/azure-dynamic-sections/hero-section";
import ContentSectionWithImage from "@/components/azure-dynamic-sections/exam-details";
import AzureFocusedContentSection from "@/components/azure-dynamic-sections/azure-focused-content-section";
import CertOverviewSection from "@/components/azure-dynamic-sections/cert-overview-section";
import FaqSection from "@/components/azure-dynamic-sections/faq-section";
...
export interface AzureFocusedContentSectionData {
...
export async function generateStaticParams() {
  const pages = await client.fetch(azureExamServicePathQuery);
...
export async function generateMetadata({ params }: { params: Params }) {
  ...
  const page = await sanityFetch<ExamServicePageData>({
    query: azureExamServicePageQuery,
    params: { slug },
  });

  if (!page) return {};

  const url = `https://testhelpnow.com/pay-someone-to-take-azure-exam-for-me/${page.slug}`;
...
async function renderSection(section: PageSection, index: number) {
  switch (section._type) {
    ...
    case "azureFocusedContentSection":
      return (
